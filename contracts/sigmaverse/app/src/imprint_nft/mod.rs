...
impl ImprintState {
    pub fn new() -> Self {
        Self {
            current_imprint_id: TokenId::from(1000000),
            ..Default::default()
        }
    }
}
...
impl ImprintNFTService {
    ...
    pub fn mint(&mut self, race: CyborRace) {
        ...
        if imprint_state.metadata_by_id.len() as u32 >= self.max_supply() {
        ...
        if let Some(template) = imprint_temp {
            ...
            // dynamic_by_id
            let story_vec = template.story.split(",").collect::<Vec<&str>>();
            let random_story_idx = block_num % story_vec.len() as u32;

            imprint_state.dynamic_by_id.insert(
                tid,
                ImprintDynamic {
                    lumimemories: 0,
                    open_story: vec![random_story_idx],
                    start_at: 0,
                },
            );
    ...
    pub fn combine(&mut self, token_id: TokenId) {
        let owner_by_id = Storage::owner_by_id();
        if let Some(&owner) = owner_by_id.get(&token_id) {
            if msg::source() == owner {
                ...
                // 先获取所需的信息
                let template = imprint_state.metadata_by_id.get(&token_id)
                    .map(|metadata| &metadata.imprint_template)
                    .unwrap();
                let story_len = template.story.split(",").collect::<Vec<&str>>().len() as u32;
                
                // 然后修改 dynamic_by_id
                if let Some(dynamic) = imprint_state.dynamic_by_id.get_mut(&token_id) {
                    if dynamic.lumimemories < 10512000 {
                        panic("ImprintNFT: not enough lumimemories");
                    } else {
                        dynamic.lumimemories -= 10512000;
                        let random_num = self.get_random_u32() % story_len;
    
                        if dynamic.open_story.contains(&random_num) {
                            panic("ImprintNFT: upgrade failed");
                        }
    
                        dynamic.open_story.push(random_num);

                        let evt = ImprintEvents::Combine {
                            from: msg::source(),
                            value: token_id,
                        };
                        let _ = self.notify_on(evt);
    ...
    pub fn burn(&mut self, token_id: TokenId) {
        let from = msg::source();
        let owner_by_id = Storage::owner_by_id();
        if let Some(&token_owner) = owner_by_id.get(&token_id) {
            if token_owner != from {
                panic("ImprintNFT: token not owned by sender");
            }
        } else {
            panic("ImprintNFT: token has not been minted");
        }

        let imprint_state = self.state.borrow();
        let _metadata = imprint_state.metadata_by_id.get(&token_id).unwrap();
