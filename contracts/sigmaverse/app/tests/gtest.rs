...
async fn mint_cybor_works() {
    ...
    println!("sigmaverse_program_id:::::: {:?}", sigmaverse_program_id);
    ...
    let result = cybor_client.get_template(CyborRace::Nguyen).send_recv(sigmaverse_program_id).await.unwrap();

    // println!("Cybor template:::::: {:?}", result);
